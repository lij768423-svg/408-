...
一个无需前端构建即可运行的桌面端 408 网页题库，覆盖刷题、答案复盘、全文搜索、学习统计、间隔复习、AI 讲题和 Markdown 个人知识库。

当前界面以桌面浏览器为主要使用环境，推荐宽度不低于 `1280px`；移动端不是本轮设计和文档覆盖范围。
...
| 刷题 | 顺序、随机、今日复习、错题、收藏、未做和跨科目随机；支持单选、多选、即时判分、结构化答案反馈和题目解析 |
| 搜索 | 搜索题干、选项和解析；按真题、模拟题、课后题及年份筛选；支持 `Ctrl/Cmd + K`、方向键选择和回车跳转 |
| 学习仪表盘 | 首次使用提供 20 题随机诊断；支持重点章节、待复习、有错题、全部章节筛选，以及多种排序和渐进展开 |
| 间隔复习 | 答错后进入计划；答对后按 1、3、7、14、30、60 天延长间隔；连续答对后移出普通错题列表 |
| AI 讲题 | 默认收起并记忆偏好；可复制当前题目上下文，或在完整后端模式下流式追问、分析错因并保存笔记 |
...
1. 第一次使用可进入“学习仪表盘”，点击“开始诊断”完成 20 道随机题，建立第一组学习样本。
2. 也可以直接进入“刷题”，选择科目、章节、模式和题量开始学习。
3. 选择答案并提交；结果区会集中显示作答状态、正确答案、你的答案和题库解析。
4. 答错时可点击“去 AI 分析错因”；不需要 AI 时直接使用唯一主操作“下一题”。
5. 回到仪表盘，根据重点章节、待复习或有错题筛选继续练习。
6. 需要长期保留时，明确选择“保存概念”或“保存题目”。
7. `LOCAL` 用户建议定期导出学习记录，避免浏览器数据被清理后丢失。

详细按钮说明、键盘操作、状态含义和常见问题见 [`docs/USAGE.md`](docs/USAGE.md)，线上也可打开应用内“使用说明”。

![桌面端刷题界面：按科目、章节和模式生成题单](docs/screenshots/quiz-desktop.png)

![答题反馈：明确标记正确项、错误项、你的答案和下一步操作](docs/screenshots/answer-feedback-desktop.png)

## 学习仪表盘与复习

首次进入学习仪表盘且没有答题记录时，页面会建议先完成 20 道随机诊断题。建立样本后，可以查看今日复习、学习进度、近期节奏和优先复习章节。

![首页介绍：账号入口、学习进度和核心能力](docs/screenshots/main.png)

![学习仪表盘：今日复习、学习节奏、优先章节和章节筛选](docs/screenshots/dashboard-desktop.png)

章节明细默认展示最需要关注的 8 行，避免一次铺满全部章节。可按“重点章节 / 待复习 / 有错题 / 全部章节”筛选，并按推荐优先级、完成度、正确率或待复习数排序；需要全量检查时再点击“查看全部”。
...
AI 面板默认收起，展开状态会保存在当前浏览器；登录后也会随学习偏好同步。面板会自动整理当前题目的科目、章节、题干、选项、用户选择、正确答案和解析。

- `LOCAL` 模式可复制 Markdown 上下文到任意外部 AI。
- 完整后端模式通过同源 `POST /api/ai/chat` 流式回答，生成中再次点击按钮可停止。
- 答错后的结果区提供“去 AI 分析错因”，会带入你的选择和正确答案。
...
- **为什么右侧没有展开 AI？** AI 面板默认收起；点击页面右侧“AI 讲题”即可展开，状态会自动记忆。

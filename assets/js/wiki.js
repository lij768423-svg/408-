...
function wikiSubjectHint(subject) {
  return subject === "数据结构" ? "算法 / 结构"
    : subject === "操作系统" ? "进程 / 内存"
    : subject === "计算机组成原理" ? "CPU / 存储"
    : subject === "计算机网络" ? "协议 / 分层"
    : subject === "数学" ? "公式 / 推导"
    : "补充笔记";
}
          ...
          '<div class="wiki-welcome-chart-wrap">' +
            '<div class="wiki-welcome-chart-stage">' +
              ...
              '<div class="wiki-welcome-chart-core">' +
                ...
                '<b data-wiki-chart-subject-label>' + escHtml(focusSubject) + '</b>' +
                '<span data-wiki-chart-description>' + escHtml(chartSummary) + '</span>' +
              '</div>' +
            '</div>' +
            chartLegend +
          '</div>' +
